...
| `BuildMotions.html` | WP-MB | 部分落地 | `createMotion_*`、`src/webview/main.ts`、`packages/three/src/MotionVisualizer.ts`、`packages/three/src/ThreeViewer.ts`、`docs/prd/scenarios/SC06_PRD_添加驱动_2026-03-11.md` | `test/step-import-selection.test.mjs`、`packages/three/test/MotionVisualizer.spec.ts`、`docs/qa/plans/SC06_QA_添加驱动_2026-03-11.md` | 已补驱动草稿预览、已创建驱动绘制图形、joint 兼容性校验、错误面拾取提示修复；已创建驱动改为红色，平移驱动改为与平移副轴线平行共轴的单向箭头，转动驱动改为顶部朝上且头部稳定位于右下象限、带双线箭头头部的弧形箭头轮廓；仍未实现帮助页级独立驱动操纵器 |
