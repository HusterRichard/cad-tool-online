...
### 5. 浏览器方式启动（Standalone Web）

当你只想复现 webview 侧问题、跑 Playwright、或不经过 VSCode 直接验证前端行为时，可以启动浏览器版壳页：

```bash
pnpm web
```

默认会先执行 `pnpm build:webview`，然后在本地启动一个静态服务：

- 入口页：`http://127.0.0.1:4174/dist/standalone/index.html`
- 根地址：`http://127.0.0.1:4174/`

Standalone Web 支持：

- 在浏览器中点击“导入”按钮，直接选择本地 `STEP / STP` 文件
- 在浏览器中导入 / 导出 `cadtool.config.json`
- 直接加载仓库内样例模型，例如：

```text
http://127.0.0.1:4174/dist/standalone/index.html?step=/ref/model/bulldozer.STEP
```

如果你只想预生成浏览器入口而不启动服务，可以执行：
...
- 该入口主要用于本地调试、自动化测试和问题复现，不替代 VSCode Extension 正式运行方式。
- 浏览器版会复用 `dist/webview/webview.js` 与 `dist/wasm/` 产物，因此需要仓库本地可读。
...
pnpm web               # 构建并启动浏览器版 standalone webview
pnpm web:prepare       # 仅生成 standalone 入口，不启动服务
