...
本项目统一使用 Conventional Commits 风格，提交描述使用中文，统一使用英文冒号 `:`。
...
- `task id` 可选，例如 `T013 `，注意后面保留一个空格
- 标题应直接说明“做了什么”，避免“一些优化”“若干修改”这类空描述
- 正文建议用要点列出关键改动与影响
...
1. 分析完整的 commit 历史，而不仅是最新一次提交。
2. 使用 `git diff [base-branch]...HEAD` 查看全部变更。
3. 编写 PR 摘要，至少包含变更概述和测试计划。
4. 新分支首次推送时使用 `git push -u origin <branch>`。
