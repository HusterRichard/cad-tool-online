...
test('group selection renders the part properties panel with group type metadata', async () => {
    ...
    const match = source.match(
        /function renderSelectedGroupProperties\(groupId: string\): void \{[\s\S]*?\n\}/
    );
...
test('group selection computes mass properties through the async worker coordinator', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const match = source.match(
        /function renderSelectedGroupProperties\(groupId: string\): void \{[\s\S]*?\n\}/
    );
    ...
    assert.ok(match, 'expected to find renderSelectedGroupProperties implementation');
    assert.match(source, /new GroupMassPropertiesCoordinator\(\s*massPropertiesWorkerClient\s*\)/);
    assert.match(match[0], /groupMassPropertiesCoordinator\.request\(/);
    assert.match(
        match[0],
        /requestAnimationFrame\(\(\) => \{\s*setTimeout\(\(\) => \{\s*const active = parseSelectionKey\(activeSelectionKey\);/
    );
    assert.doesNotMatch(
        source,
    ...
    assert.doesNotMatch(
        match[0],
        /setTimeout\(\(\) => \{\s*const massSummary = aggregateGroupMassProperties\(groupId\);/
    );
});
...
test('step import disambiguates duplicate part names before imported groups are materialized', async () => {
    ...
    assert.match(
        loadStepFileMatch[0],
        /assignUniqueImportedPartNames\(rootShapes\);[\s\S]*initializeImportedGroupDesignState\(\);/
    );
});
...
test('step import materializes exclusive imported groups for initially ungrouped parts', async () => {
    ...
    const initMatch = source.match(
        /function initializeImportedGroupDesignState\(\): void \{[\s\S]*?\n\}/
    );
...
test('model tree preserves expansion state and only seeds the imported objects root as expanded', async () => {
    ...
    assert.doesNotMatch(
        source,
        /getOrderedChildGroupIds\(null\)\.forEach\(\(groupId\) => \{\s*treeExpandedNodeIds\.add\(`group_\$\{groupId\}`\);\s*\}\);/
    );
...
test('step hierarchy import materializes assemblies into selectable imported groups', async () => {
    ...
    assert.match(
        source,
        /const memberPartIds = shape\.type === 'assembly'\s*\?\s*\(shape\.children \?\? \[\]\)\s*\.filter\(\(child\) => child\.type !== 'assembly'\)\s*\.map\(\(child\) => child\.id\)\s*:\s*\[shape\.id\];/
    );
...
test('step load initializes imported group state before model tree refresh', async () => {
    ...
    assert.match(
        loadStepFileSource,
        /rootShapes\.push\(buildShapeTree\(root\)\);[\s\S]*initializeImportedGroupDesignState\(\);[\s\S]*updateModelTree\(\);/
    );
});

test('default group creates one exclusive parent group for every ungrouped part', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const handleCreateDefaultGroupMatch = source.match(
        /function handleCreateDefaultGroup\(\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        handleCreateDefaultGroupMatch,
        'expected to find handleCreateDefaultGroup implementation'
    );
    assert.match(handleCreateDefaultGroupMatch[0], /const createdGroups = memberShapeIds/);
    assert.match(
        handleCreateDefaultGroupMatch[0],
        /createGroupFromParts\(shape\.name, \[shape\.id\], null, 'default'\)/
    );
    assert.doesNotMatch(
        handleCreateDefaultGroupMatch[0],
        /createGroupFromParts\(`DefaultGroup\$\{mbsGroups\.size \+ 1\}`, memberShapeIds, null, 'default'\)/
    );
});
