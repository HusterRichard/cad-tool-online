...
test('step import keeps the initial selection empty', async () => {
    ...
    const loadStepFileMatch = source.match(
        /async function loadStepFile\(fileName: string, fileContent: unknown\): Promise<void> \{[\s\S]*?\n\}/
    );
...
test('marker creation opens the options panel without requiring a preselected part', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const startMarkerCreationMatch = source.match(
        /function startMarkerCreation\(\): void \{[\s\S]*?\n\}/
    );
...
test('reference marker creation opens options panel and requires an existing base marker', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const startRefFrameCreationMatch = source.match(
        /function startRefFrameCreation\(\): void \{[\s\S]*?\n\}/
    );
    ...
    assert.match(
        startRefFrameCreationSource,
        /Please create a basic marker first before creating a reference marker\./
    );
...
test('reference marker creation narrows 3D picking to markers first, then target parts after base selection', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const viewerSource = await readFile(
        new URL('../packages/three/src/ThreeViewer.ts', import.meta.url),
        'utf8'
    );
    const selectionManagerSource = await readFile(
        new URL('../packages/three/src/SelectionManager.ts', import.meta.url),
        'utf8'
    );

    assert.match(source, /function updateRefFrameSelectionFilter\(\): void \{/);
    assert.match(
        source,
        /if \(!refFrameCreationPanelActive\) \{\s*viewer\?\.setSelectionFilter\(null\);/
    );
    assert.match(
        source,
        /if \(!pendingRefFrameBaseId\) \{\s*viewer\?\.setSelectionFilter\(id => isMarkerId\(id\)\);/
    );
    assert.match(source, /viewer\?\.setSelectionFilter\(id => meshIdToShapeId\.has\(id\)\);/);
    assert.match(source, /pendingRefFrameBaseId = selectedMarkerId;/);
    assert.match(source, /updateRefFrameSelectionFilter\(\);/);
    assert.match(viewerSource, /setSelectionFilter\(filter: SelectionFilter \| null\): void \{/);
    assert.match(
        selectionManagerSource,
...
test('marker creation wires hover preview updates on canvas pointer move', async () => {
    ...
    assert.doesNotMatch(
        selectionManagerSource,
        /pickObjectIdAtScreenPoint\(x: number, y: number\): string \| null \{\s*if \(!this\.enabled\)/
    );
});

test('frame editing reuses hover inference while suppressing normal scene selection', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const startFrameEditModeForTargetMatch = source.match(
        /function startFrameEditModeForTarget\([\s\S]*?\n\}/
    );
    const handleCanvasPointerMoveMatch = source.match(
        /function handleCanvasPointerMove\(event: MouseEvent\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        startFrameEditModeForTargetMatch,
        'expected to find startFrameEditModeForTarget implementation'
    );
    assert.ok(
        handleCanvasPointerMoveMatch,
        'expected to find handleCanvasPointerMove implementation'
    );
    ...
    assert.match(
        handleCanvasPointerMoveSource,
        /canvasInteractionMode === 'editFrame' && editingFrameTarget !== null/
    );
    assert.match(handleCanvasPointerMoveSource, /viewer\?\.setMarkerGuide\(placement\.guide\)/);
    assert.match(
        handleCanvasPointerMoveSource,
        /if \(markerPlacementActive\) \{\s*showMarkerPreview/
    );
});

test('marker creation stays in marker placement mode after creating a marker', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const handleCanvasClickMatch = source.match(
        /function handleCanvasClick\(event: MouseEvent\): void \{[\s\S]*?\n\}/
    );
    const finalizeMarkerDraftMatch = source.match(
        /function finalizeMarkerDraft\(\): void \{[\s\S]*?\n\}/
    );
...
test('marker tree context menu supports deleting marker nodes', async () => {
    ...
    assert.match(
        source,
        /if \(nodeData\.frameId\) \{\s*return \[\s*\{ label: '.*', action: 'deleteFrame' \}/
    );
});
...
test('reference marker fast mode auto-creates after selecting target part', async () => {
    ...
    assert.match(
        source,
        /if \(refFrameCreationMode === 'fast' && pendingRefFrameBaseId\) \{\s*createReferenceFrameFromSelection\(pendingRefFrameBaseId, selectedShapeId\);/
    );
});

test('reference marker fast mode resets to the initial workflow and hovers the original base marker after creation', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const viewerSource = await readFile(
        new URL('../packages/three/src/ThreeViewer.ts', import.meta.url),
        'utf8'
    ...
    const createReferenceFrameFromSelectionMatch = source.match(
        /function createReferenceFrameFromSelection\(baseMarkerId: string, targetShapeId: string\): boolean \{[\s\S]*?\n\}/
    );

    assert.ok(
        createReferenceFrameFromSelectionMatch,
        'expected to find createReferenceFrameFromSelection implementation'
    );

    const fnSource = createReferenceFrameFromSelectionMatch[0];
    assert.match(fnSource, /if \(refFrameCreationMode === 'fast'\) \{/);
    assert.match(fnSource, /pendingRefFrameBaseId = null;/);
    assert.match(fnSource, /pendingRefFrameTargetShapeId = null;/);
    assert.match(fnSource, /selectSelection\(null\);/);
    ...
    assert.match(fnSource, /viewer\?\.setHoveredId\(relatedMarker\.id\);/);
    assert.match(viewerSource, /setHoveredId\(id: string \| null\): void \{/);
    assert.match(selectionManagerSource, /setHoveredId\(id: string \| null\): void \{/);
});

test('reference marker standard mode validates base and target before confirm creation', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const renderRefFrameCreationPanelMatch = source.match(
        /function renderRefFrameCreationPanel\(\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        renderRefFrameCreationPanelMatch,
        'expected to find renderRefFrameCreationPanel implementation'
    );
...
test('reference marker panel asks users to pick the base marker from 3D selection', async () => {
    ...
    assert.match(
        panelSource,
        /当前基本标架：\$\{selectedBaseMarker\?\.name \?\? '\(未选择，请在三维中拾取标架\)'\}/
    );
...
test('marker properties panel exposes editable name visibility size position and direction', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const renderMarkerPropertiesPanelMatch = source.match(
        /function renderMarkerPropertiesPanel\(markerId: string\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        renderMarkerPropertiesPanelMatch,
        'expected to find renderMarkerPropertiesPanel implementation'
    );
...
test('reference marker properties panel keeps core fields read-only and only edits visibility', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const renderRefFramePropertiesPanelMatch = source.match(
        /function renderRefFramePropertiesPanel\(refFrameId: string\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        renderRefFramePropertiesPanelMatch,
        'expected to find renderRefFramePropertiesPanel implementation'
    );
...
test('reference marker deletion detaches from base marker and clears entity map', async () => {
    ...
    assert.match(
        source,
        /if \(refFrame\.relatedMarkerId\) \{\s*const parentMarker = createdMarkers\.find\(\(marker\) => marker\.id === refFrame\.relatedMarkerId\);\s*parentMarker\?\.removeRefMarker\(refFrame\.id\);/
    );
...
test('frame visualizer adds a circular ring around the marker triad', async () => {
    const frameVisualizerSource = await readFile(
        new URL('../packages/three/src/FrameVisualizer.ts', import.meta.url),
        'utf8'
    );
...
test('frame selection uses dedicated accent colors instead of the generic pale highlight overlay', async () => {
    ...
    assert.match(
        selectionManagerSource,
        /if \(object\.userData\.selectionAppearance === 'frame'\) \{\s*return;\s*\}/
    );
...
test('marker face placement converts viewer rays through the shape transform', async () => {
    ...
    assert.match(
        resolveFacePlacementSource,
        /viewer\.getRayFromScreenPoint\(event\.clientX,\s*event\.clientY\)/
    );
    assert.match(resolveFacePlacementSource, /invertRigidTransform\(shapeTransform\)/);
    assert.match(
        resolveFacePlacementSource,
        /transformPoint\(\s*inverseShapeTransform,\s*ray\.origin\.x/
    );
    assert.match(
        resolveFacePlacementSource,
        /transformDirection\(\s*inverseShapeTransform,\s*ray\.direction\.x/
    );
    assert.match(
        resolveFacePlacementSource,
        /transformPoint\(\s*shapeTransform,\s*result\.position\.x/
    );
    assert.match(
        resolveFacePlacementSource,
        /transformDirection\(\s*shapeTransform,\s*result\.normal\.x/
    );
    assert.match(
        resolveFacePlacementSource,
...
test('cad editor panel serves ribbon icons from svg assets', async () => {
    const panelSource = await readFile(
        new URL('../src/panels/CadEditorPanel.ts', import.meta.url),
        'utf8'
    );
...
test('model tree icons resolve svg files instead of raster png files', async () => {
    ...
    assert.match(
        source,
        /case 'category_objects':\s*return treeIconPath\('model_tree_body_dir\.png'\)/
    );
    assert.match(
        source,
        /case 'category_connections':\s*return treeIconPath\('model_tree_connector_dir\.png'\)/
    );
    assert.match(
        source,
        /case 'category_motions':\s*return treeIconPath\('model_tree_motion_dir\.png'\)/
    );
    assert.match(
        source,
        /case 'category_forces':\s*return treeIconPath\('model_tree_force_dir\.png'\)/
    );
    assert.match(
        source,
        /case 'category_materials':\s*return treeIconPath\('model_tree_material_dir\.png'\)/
    );
});
...
test('step reader enables color transfer and resolves colors through referenced shapes', async () => {
    const geoBindingSource = await readFile(
        new URL('../packages/geo/cpp/src/geo/geo_binding.cpp', import.meta.url),
        'utf8'
    );
...
test('cylindrical marker hover inference exposes explicit cylinder-axis snap metadata', async () => {
    ...
    const geoTypesSource = await readFile(
        new URL('../packages/geo/src/types.ts', import.meta.url),
        'utf8'
    );
    ...
    assert.match(
        geoBindingSource,
        /BRepTools::UVBounds\(closestFace,\s*uMin,\s*uMax,\s*vMin,\s*vMax\);/
    );
...
test('model tree uses icon-based expand toggles and compact unified icon sizing', async () => {
    ...
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const expandIcon = await readFile(
        new URL('../public/icons/svg/16/model_tree_cad_expand.svg', import.meta.url),
        'utf8'
    );
    const collapseIcon = await readFile(
        new URL('../public/icons/svg/16/model_tree_cad_collapse.svg', import.meta.url),
        'utf8'
    );
    ...
    assert.match(
        panelSource,
        /\.tree-node\.tree-node-category \.icon \{\s*width: 11px;\s*height: 11px;/
    );
    assert.match(
        panelSource,
        /\.tree-node\.tree-node-category \.icon img \{\s*width: 11px;\s*height: 11px;/
    );
    assert.doesNotMatch(panelSource, /\.tree-node \.expand-btn::before \{/);
    assert.match(
        panelSource,
        /\.tree-node \.expand-btn \{\s*[\s\S]*background-color: transparent;/
    );
    ...
    assert.match(
        source,
        /if \(nodeData\.kind === 'category'\) \{\s*node\.classList\.add\('tree-node-category'\);/
    );
...
test('motion and joint model tree nodes carry selection keys for direct selection', async () => {
    ...
    assert.match(
        source,
        /motionsCategory\.children = Array\.from\(mbsMotions\.values\(\)\)\.map\(\(motion\) => \(\{[\s\S]*selectionKey: toMotionSelectionKey\(motion\.id\)/
    );
});

test('motion options panel exposes fast standard creation modes and connector-driven controls', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const renderMotionOptionsPanelMatch = source.match(
        /function renderMotionOptionsPanel\(\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        renderMotionOptionsPanelMatch,
        'expected to find renderMotionOptionsPanel implementation'
    );
...
test('motion properties panel exposes editable drive fields while keeping the motion type read-only', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const renderMotionPropertiesPanelMatch = source.match(
        /function renderMotionPropertiesPanel\(motionId: string\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        renderMotionPropertiesPanelMatch,
        'expected to find renderMotionPropertiesPanel implementation'
    );
...
test('motion creation import and export persist drive-specific fields', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const createMotionFromDraftMatch = source.match(
        /function createMotionFromDraft\(\): boolean \{[\s\S]*?\n\}/
    );
...
test('joint creation exposes Alt plus mouse wheel icon sizing affordance', async () => {
    const source = await readFile(new URL('../src/webview/main.ts', import.meta.url), 'utf8');
    const renderJointOptionsPanelMatch = source.match(
        /function renderJointOptionsPanel\(\): void \{[\s\S]*?\n\}/
    );

    assert.ok(
        renderJointOptionsPanelMatch,
        'expected to find renderJointOptionsPanel implementation'
    );
...
test('delete selection handles joints alongside motions contacts and design points', async () => {
    ...
    assert.match(
        source,
        /selectedJointIds\.forEach\(\(jointId\) => \{\s*if \(deleteJointById\(jointId\)\) \{\s*deletedJointCount \+= 1;/
    );
    assert.match(
        source,
